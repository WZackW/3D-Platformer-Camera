...
void UNewSpringArm::UpdateDesiredArmLocation(bool bDoTrace, bool bDoLocationLag, bool bDoRotationLag, float DeltaTime)
{
	...
	FVector ArmOrigin = TargetOffset;
	if(FollowZ)
	{
		ArmOrigin += GetComponentLocation();
	}
	else 
	{
		ArmOrigin += FVector(GetComponentLocation().X, GetComponentLocation().Y,10);
	}
	...
	FVector ResultLoc;
	if ((bDoTrace || CameraHasCollided) && (TargetArmLength != 0.0f))
	{
		bIsCameraFixed = true;
...
void UNewSpringArm::SetZLookLag(float Val)
{
	CameraLagSpeed = Val;
}
